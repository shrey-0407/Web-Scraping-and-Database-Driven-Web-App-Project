...
import logging
import sys
sys.path.append('D:\Web_scrapping\python-azlyrics_scrapper')

from bin.proxies import Proxy_Checker
            ...
            try:
                ...
                if site.status_code == requests.codes.ok:
                    ...
                    if len(html_soup.find_all('div', class_='alert alert-info')) == 0:
                        ...
                        self.az_logger.debug('Recaptcha Detected -> {} using Proxy -> {} on try-> {}'.format(url, proxy, flag))
                        flag -= 1
                        if flag == 0:
                            return '0'
                else:
                    ...
                    try:
                        ...
                        if html_soup != '0':
                            ...
                            for album in albums:
                                ...
                                album_year = re.findall(r"\((\d+)\)", album.text)[0] if re.search(r"\((\d+)\)", album.text) else 'Not Defined'
                        ...
                        self.az_logger.warning('Something Went Wrong while Requesting -> {} Error -> {} on line No. -> {}'.format(url, E, sys.exc_info()[-1].tb_lineno))
            temp_flag -= 1
    ...
    async def get_batch_lyrics(self):
        ...
        while flag:
            try:
                ...
                self.save_az_lyrics()
        self.az_logger.info(f'Scrapping Successful... No. of Lyrics in file is {len(self.az_lyrics["completed"])}')
